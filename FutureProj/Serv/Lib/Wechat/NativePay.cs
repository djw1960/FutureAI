...
* @return 模式一URL
*/
        public string GetPrePayUrl(WxPayConfig config, string productId)
        {
        ...
        * @return 模式二URL
        */
        public string GetPayUrl(WxPayConfig config,string productId)
        {
            WxPayData data = new WxPayData();
            data.SetValue("body", "test");//商品描述
            data.SetValue("attach", "test");//附加数据
            data.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo(config));//随机字符串
            data.SetValue("total_fee", 1);//总金额 单位 分
